...
shinyServer(function(input, output) {
    ...
    earthIcon <- icons(iconUrl = "www/earth.png", iconHeight = .02 * log(earthOriginalSize), iconWidth = .02 * log(earthOriginalSize))
    ...
    planetsScaledSize <- reactive({((planetsOriginalSize * 1000 * 100) / AU) * planetScale()}) #converting km to m and then to cm
    earthScaledSize <- reactive({((6378.1 * 2 * 1000 * 100) / AU) * planetScale()}) #see above, but for Earth seperately
    ...
    dataTab <- reactive({data.table(Planet = planetNames, #This constructs a table of all data that is necessary to then go out into the world and place the planet models, as well as to know what size the models to build; you should probably refrain from building a to-scale model of the sun, as it will necessarily be huge, no matter what scale you choose.
                                    ...
                                    "Scaled size in cm" = round(planetsScaledSize(), digits = 2)) %>%
    ...
    output$solarSystemMap <- renderLeaflet({ #Rendering the actual map
        leaflet() %>%
        addTiles() %>%
        addMarkers(lng = startingLocationLng(), lat = startingLocationLat(), label = "earth", icon = earthIcon) %>%
        addMarkers(lng = newCoords()[,1], lat = newCoords()[,2], label = planetNames, icon = planetIcons)
    })
